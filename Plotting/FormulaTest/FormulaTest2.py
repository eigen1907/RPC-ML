...
def plotFormularVSIoverL(rpcImonPath, plotPath, dpidName):    
    ...
    ax1 = plt.subplot(221)
    ax1.plot(date, I/L, '.', label="Imon/inst_lumi")
    ax1.set_xlabel("Date")
    ax1.legend()
    ...
    plt.figure(figsize=(16, 8))
    plt.plot(formula, V, ".")
    plt.xlabel("Formula(Veff)")
    plt.ylabel("Vmon(Vapp)")
    plt.title(f"After Separate\n{dpidName[0:-4]}\nVmon(V_app) / Formula = V_eff")
...
if __name__ == "__main__":
    rpcImonPath = "/store/hep/users/eigen1907/CMS-RPC_store/DataStore/ImonPreproc/GoldenRPCSeparate/"
    plotPath = "/store/hep/users/eigen1907/CMS-RPC_store/PlotStore/Preprocessing/FormulaTest2/"
